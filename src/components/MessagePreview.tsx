...
const CHAT_BACKGROUND_PATTERN = `url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%231f2c34' fill-opacity='0.4'%3E%3Cpath d='M11 18c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm48 25c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm-43-7c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm63 31c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zM34 90c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm56-76c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zM12 86c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm28-65c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm23-11c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm-6 60c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm29 22c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zM32 63c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm57-13c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm-9-21c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zM60 91c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zM35 41c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zM12 60c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2z' /%3E%3C/g%3E%3C/svg%3E")`;
...
export function MessagePreview({
  ...
  const hasContent = message.trim() || mediaUrl;
    ...
    <div className="bg-[#0b141a] rounded-2xl border border-zinc-800 overflow-hidden shadow-2xl">
      <div className="px-3 py-2.5 bg-[#1f2c34] border-b border-[#2a3942] flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-medium text-zinc-100 truncate">{displayName}</h3>
      ...
      <div
        ...
        style={{ backgroundColor: "#0b141a", backgroundImage: CHAT_BACKGROUND_PATTERN }}
      >
        {hasContent ? (
          <motion.div
          ...
          >
            <div className="relative max-w-[85%] sm:max-w-[75%]">
              <div
                ...
                style={{ background: "linear-gradient(135deg, #005c4b 50%, transparent 50%)" }}
              />

              <div className="rounded-lg rounded-tr-sm px-3 py-2 shadow-md" style={{ backgroundColor: "#005c4b" }}>
                {mediaUrl && (
                  <div className="mb-2 -mx-1 -mt-1">
                    {mediaType === "image" ? (
                      <img src={mediaUrl} alt="Media" className="rounded-lg max-h-40 object-cover" />
                    ) : (
                ...
                <div className="flex items-center justify-end gap-1 mt-1">
                  <span className="text-[10px] text-zinc-400">{formatTime()}</span>
          ...
          <div className="text-center">
            <div className="inline-flex flex-col items-center gap-3 p-6 rounded-2xl bg-zinc-800/50">
              ...
              <p className="text-xs text-zinc-600">Type a message to see how it will look</p>
            </div>
          </div>
        )}
      </div>
